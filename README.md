...
本项目仅供学习、测试与技术分享，防火墙、白名单、入站限制可能违反云厂商或机房的服务条款，存在封禁、停机、扣款等风险，本项目不提供任何担保。
...
- **过期拆除**：信箱 48 小时未再上报的 IP，下次拉取会从 `po0_client_ips` 拆掉；省市 CIDR 与「添加 IP」留下的地址不动。
- **应用**：有省市、有客户端 IP、或两个都有，都会挂同一条链：命中任一来源放行，其余拒绝。只配信箱/手动加 IP，不必先选省市。未配信箱时不会安装拉取定时器。
