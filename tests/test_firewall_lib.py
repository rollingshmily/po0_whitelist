...
class FirewallLibTests(unittest.TestCase):
    ...
    def test_install_script_exposes_setup_and_p_shortcut(self):
        ...
        self.assertIn('ipset flush "${PO0_CLIENT_SET_NAME}"', script)
        self.assertIn("apply_manual_ips()", script)
        self.assertIn("remember_manual_ip()", script)
        self.assertIn("过期地址已拆除", script)
    ...
    def test_mailbox_allow_list_splits_mixed_separators(self):
        ...
        self.assertIn("refresh_update_archive", source)
    ...
    def test_github_fetch_uses_china_mirrors(self):
        ...
        self.assertIn("po0_fetch_via_mailbox", fetch)
        self.assertIn("/update", fetch)
        self.assertIn("已配信箱时不会改走 GitHub 加速源", fetch)
